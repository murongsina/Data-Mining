function [ yTest, Time, W ] = MTL_TWSVR_New(xTrain, yTrain, xTest, opts)
...
%% Fit
    ...
    % MTL_TWSVR1
    ub1 = e*C1;
    Alpha = quadprog(Q+rho*P,g-H'*f,[],[],[],[],lb,ub1,[],solver);
    % MTL_TWSVR2
    ub2 = e*C2;
    Gamma = quadprog(Q+lambda*P,H'*g-f,[],[],[],[],lb,ub2,[],solver);
...
%% GetWeight
    ...
    for t = 1 : TaskNum
        Tt = T==t;
        Ut = AAAt{t}*(f(Tt,:) - rho*Alpha(Tt,:));
        Vt = AAAt{t}*(g(Tt,:) + lambda*Gamma(Tt,:));
        Uts = U + Ut;
        Vts = V + Vt;
        W{t} = (Uts + Vts)/2;
    end
